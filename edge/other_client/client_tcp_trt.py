...
def _listen(edgehost, edgeport, listen_queue):
    ...
    while True:
        ...
        if data[-4:] == b'\\EOF':  # marker=2的包
            stick_packet = data[:-24]  # 取出和marker为2的包粘一起的包
            ...
            data_array = bytearray()
            counter = 0
            listen_queue.put((data[-24:-4], 1, 1))
        else:
...
class EdgeClient:
    def __init__(self, args):
        ...
        self.model_dic = {'01': '/res/yolov5/weights/5n_b32_e20_p.engine',
                          '02': '/res/yolov5/weights/5s_b32_e20_p.engine',
    ...
    def _count(self, transmission_time, decode_time):
        ...
        print("inference====", np.mean(inference_time))
        print("acc====", np.mean(acc_time))
    ...
    def decode(self, data, all_Payload, all_Sequence, all_num):
        ...
        if currentMarker == 0:  # 这个包为数据的第一个包
            ...
            in_bytes = decode_process.stdout.read(854 * 480 * 3 * 30)
            frame = bytes2numpy(30, in_bytes, 480, 854)
            decode_time.append(time.time() - start)

            self.inference = threading.Thread(target=self._edge_inference, args=(timestamp, frame, device_ID, video_ID, chunk_ID))
    ...
    def _edge_inference(self, timestamp, frame, device_ID, video_ID, chunk_ID):
        ...
        for num in range(rounds):
            inference_data = frame[num][None, :]
            torch.tensor(inference_data).to(self.device)
            start_time = time.time()
            pred = self.yolo.inference(inference_data)
            inference_time_sum.append(time.time() - start_time)
        # 精度评估
            acc_start = time.time()
            if len(labels_chunk[num]) == 0:  # label文件为空
                if len(pred) == 0:  # 推理结果为空
                    acc_chunk.append(1)
                else:
                    acc_chunk.append(0)
            else:
                if len(pred) == 0:
                    ...
                    if (len(np.shape(labels_chunk[num]))) == 1:
                        labels_chunk[num] = labels_chunk[num][None, :]
                    labels_chunk[num][:, 1:5] = xywh2xyxy(labels_chunk[num][:, 1:5]) * (
                        [854, 480, 854, 480])  # target boxes
                    stats = self.process_batch_acc(pred[0].cpu(), torch.tensor(labels_chunk[num]), iouv)
                    status = stats.cpu().numpy()[:, 0]
                    if len(status) != 0:
                        acc_chunk.append(status.mean())
            acc_time_sum.append(time.time() - acc_start)
        ...
        MQ_dic = {
            ...
            # 'acc_chunk': 0.8584,  # 该chunk的精度评估结果
        ...
        inference_time.append(np.sum(inference_time_sum))
        acc_time.append(np.sum(acc_time_sum))
