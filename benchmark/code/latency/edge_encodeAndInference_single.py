...
class EdgeClient:
    def __init__(self, args):
        ...
        self.edge_inference = threading.Thread(target=self._edge_inference, daemon=True)
        self.edge_inference.start()

        self.load_camera_data = threading.Thread(target=self._load_camera_data)
        self.load_camera_data.start()
        time.sleep(200)
        print("over")
        with open("edge_encodeAndInference_single.txt", "a") as file:
            file.write(
                "modelID:" + self.modelID + "  batchsize = " + str(
                    self.batchsize) +
                "  inference length = " + str(len(self.inference_latency)) + "  edge_inference latency = " + str(
        ...
        for i in range(10):
            ...
            while j < num_chunks:
                if j < num_local_chunk:
                    print('put to local')
                    _process_inference_process(inference_queue, stream, labels, i, j, self.device_ID)
                j += 1
                time.sleep(1)
