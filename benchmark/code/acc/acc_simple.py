...
class EdgeClient:
    ...
    def _process_h264Tobgr24(self):  # 处理listen得到的数据
        ...
        while i < len(self.camera_dataset):
            ...
            i = i + 1
        time.sleep(1)
        with open("dataset_acc_v8.txt", "a") as file:
            file.write(
                self.resolution + ', ' + self.bitrate + ', ' + self.modelimgsz + ', ' + str(np.mean(All_acc)) + ', ' + str(np.mean(latency[1:])*2) + '\n')
    ...
    def _edge_inference(self, frame_chunk):
        ...
        for num in range(rounds):
            ...
            start = time.time()
            pred = self.yolo.predict(inference_data, half=True, imgsz=self.model_inputsize[0])
