...
class EdgeClient:
    def __init__(self, args):
        ...
        self.path = ''
        if self.modelID == 0:
            self.path = '/data0/yubo.xuan/ultralytics/runs/detect/yolov8_m_lr0.01/weights/'
        elif self.modelID == 1:
            self.path = '/data0/yubo.xuan/ultralytics/runs/detect/yolov8_s_lr0.01/weights/'
        elif self.modelID == 2:
            self.path = '/data0/yubo.xuan/ultralytics/runs/detect/yolov8_s_lr0.001/weights/'
        elif self.modelID == 3:
            self.path = '/data0/yubo.xuan/ultralytics/runs/detect/yolov8_n_lr0.01/weights/'
        elif self.modelID == 4:
            self.path = '/data0/yubo.xuan/ultralytics/runs/detect/yolov8_n_lr0.001/weights/'
        else:
            self.path = '/data0/yubo.xuan/ultralytics/runs/detect/yolov8_n_yolo_train/weights/'
        model_path = self.path + self.model_name + '.pt'
    ...
    def _process_h264Tobgr24(self):  # 处理listen得到的数据
        ...
        while i < len(self.camera_dataset):
            ...
            while j < num_chunks:
                ...
                j += 1
            acc_list.append(np.mean(self.video_acc))
            i = i + 1
        time.sleep(1)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--modelID", required=False, default=0, type=int,
    ...
    parser.add_argument("--modelimgsz", required=False, default='320x320', type=str,
                        help='the encode bitrate')
    parser.add_argument("--model_name", required=False, default='best', type=str,
                        help='the epoch')
