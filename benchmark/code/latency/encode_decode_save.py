    ...
    def _process_h264Tobgr24(self):  # 处理listen得到的数据
        ...
        while i < len(self.camera_dataset):
            ...
            while j < num_chunks:
                ...
                # print("encode_time", time.time() - encode_time)
                # file_path = './encode_decode_data/' + str(self.bitrate) + '_' + self.resolution + '.pkl'
                # with open(file_path, 'wb') as file:
                #     pickle.dump(edge_frame, file)
                #
                ...
                #     continue
                #
                inference_process = stream.inference_process(j, resolution=self.resolution)  # 格式转化
                edge_frame = inference_process.stdout.read(width * height * 3 * 30)
                print("encode_time===", time.time() - encode_time)

                # file_path = './encode_decode_data/' + self.bitrate + '_' + self.resolution + '_' + str(i) + '_' + str(j) + '.pkl'
                # with open(file_path, 'wb') as file:
                #     pickle.dump(edge_frame, file)
                ...
                #     edge_frame = pickle.load(file)

                frame = bytes2numpy(30, edge_frame, height, width)
                if len(frame) == 30:
                    self._edge_inference((encode_time, frame, self.deviceID, data[1] + 1, data[2] + 1))
                # print("all_time==", time.time() - time_start)
                del frame
                j += 1
            i = i + 1
            # break
