...
class EdgeClient:
    def __init__(self, args):
        ...
        self.imgsz = (image[1], image[0])  # (height, width)
        self.flag = False
    ...
    def _process_h264Tobgr24(self):  # 处理listen得到的数据
        ...
        with open("9_25_acc_latency.txt", "a") as file:
            file.write(
                self.resolution + ', ' + self.bitrate + ', ' + str(np.mean(latency[2:])*2) + ', ' + str(np.mean(All_acc)) + ', ' + self.modelID + '\n')
    ...
    def _edge_inference(self, frame_chunk):
        ...
        for num in range(rounds):
            ...
            start = time.time()
            # pred_bantch = self.yolo.predict(inference_data, half=True, imgsz=(self.imgsz[1]))
            pred_bantch = self.yolo.predict(inference_data, half=True, imgsz=(self.imgsz[0]))
