            ...
            try:
                ...
                if len(data) > 0:
                    ...
                    if counter >= CATCH_SIZE:  # 加个大于号 为了防止粘包
                        listen_queue_time = time.time()
                        listen_queue.put((data_array[:CATCH_SIZE*1448], CATCH_SIZE, 0))
                        print("listen_queue_time=", time.time() - listen_queue_time)
...
def _process(all_Payload, all_Sequence, all_num, listen_queue):  # 处理listen得到的数据
    ...
    while True:
        ...
        if Is_last == 0:   # 处理的包不是最后一个包时
            ...
            for num in range(counter-1):
                ...
                decode(data, all_Payload, all_Sequence, all_num)
...
def decode(data, all_Payload, all_Sequence, all_num):
    # 解包
    ...
    if currentMarker == 0:  # 这个包为数据的第一个包
        print("Cloud get the first packet form video %d chunk %d of device %d" % (
        ...
        print("Cloud get the last packet form video %d chunk %d of device %d" % (
            video_ID, chunk_ID, device_ID))
        ...
        queue_put_time.append(time.time() - put_start)
...
class CloudServer:
    def __init__(self, args):
        ...
        self.model_dic = {'01': '/res/yolov5/weights/5n_b32_e20_p.pt',
                          ...
                          '03': '/res/yolov5/weights/5m_b32_e50_p.engine'}
        ...
        self.listen_process_pool = Pool(2)
        ...
        self.count = threading.Thread(target=self._count, args=(transmission_time, decode_time, queue_put_time),
                                      daemon=True)
        self.count.start()
    ...
    def _count(self, transmission_time, decode_time, queue_put_time):
        time.sleep(200)
        print("All_part====", np.mean(All_part))
        print("transmission====", np.mean(transmission_time))
        print("decode====", np.mean(decode_time))
        print("queue_put====", np.mean(queue_put_time))
        print("queue_get====", np.mean(queue_get_time))
    ...
    def _cloud_inference(self):
        ...
        while True:
            if inference_queue.qsize() > 0:
                ...
                queue_get_time.append(time.time() - start)
