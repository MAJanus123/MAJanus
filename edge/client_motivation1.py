...
def _process_transmission_process(transmission_queue, stream, i, j, device_ID, step_index, resolution, bitrate, width, height):
    ...
    print("encode cloud time==", time.time() - timestamp, "mean:",np.mean(offload_encode))
...
class EdgeClient:
    ...
    def _process_inference_process(self, inference_queue, stream, i, j, device_ID, step_index, resolution, width,
        ...
        print("encode edge time==", time.time() - timestamp, "mean:",np.mean(local_decode))
    ...
    def _load_camera_data(self, reset_time):
        print('start to load camera data, reset_time==', reset_time)
        i = 0
        while True:
            videoNum = i % 8
            ...
            while j < num_chunk:
                ...
                # 启动timer
                time_flag.value = 0
                self.timer.apply_async(func=timer)
                ...
                # if i == all_round-1 and j == num_chunk - 1:   # 一个dataset的最后，发送ppo标志位  正常reset
                #     message = {
    ...
    def _process(self):  # 处理listen得到的数据
        ...
        while True:
            ...
            self._edge_inference((timestamp, frame, self.edgedeviceid, videoID, chunkID, queue_wait, height, width))
    ...
    def _edge_inference(self, frame_chunk):
        ...
        for num in range(rounds):
            ...
            if self.modelID == 'm':
                ...
                pred_bantch_extra = self.yolo_m.predict(inference_data[:math.ceil(length/4)], half=True, imgsz=height)
            elif self.modelID == 's':
        ...
        print("inference time====", inferenceTime, "mean:", np.mean(inference_time))
        # 精度评估
        start_acc = time.time()
        stats = []
        ...
        process_time = time.time() - MQ_dic['timestamp']
        All_part.append(process_time)
        print("MQ_dic:  ", MQ_dic, "process_time: ", process_time, "mean: ", np.mean(All_part))
    ...
    def _control_client(self):
        ...
        connection = pika.BlockingConnection(
            pika.ConnectionParameters(self.cloudhost, self.mqport, credentials=user_info, heartbeat=0))
        ...
        def callback(ch, method, properties, body):
            ...
            if command['type'] == 'reset':
                ...
                self.reset_times += 1
                self.use_agent_ID = command['use_agent_ID']
                if self.edgedeviceid in self.use_agent_ID:
