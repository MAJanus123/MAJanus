...
if FLAG == 1:
    ##train
    seq_root = "/data0/yubo.xuan/DETRAC-dataset/DETRAC-train-data/Insight-MVT_Annotation_Train/"  # 图片
    xml_root = "/data0/yubo.xuan/DETRAC-dataset/DETRAC-Train-Annotations-XML/DETRAC-Train-Annotations-XML/"  # 原始xml标注
    label_root = "/data0/yubo.xuan/DETRAC-dataset/train_label/"  # 新生成的标签保存目录
else:
    ##test
    seq_root = "/data0/yubo.xuan/DETRAC-dataset/DETRAC-test-data/Insight-MVT_Annotation_Test/"  # 图片
    xml_root = "/data0/yubo.xuan/DETRAC-dataset/DETRAC-Test-Annotations-XML/DETRAC-Test-Annotations-XML/"  # 原始xml标注
    label_root = "/data0/yubo.xuan/DETRAC-dataset/labels/"  # 新生成的标签保存目录
...
class XmlTester(XmlReader):
    ...
    def load(self, filename):
        ...
        if None != filecontent:
            ...
            # 获取ignore区域
            ignore = root.getElementsByTagName('ignored_region')[0]
            ign_boxes = ignore.getElementsByTagName('box')
            for ign_box in ign_boxes:
                # 获取<box>标签的属性
                left = ign_box.getAttribute('left')
                top = ign_box.getAttribute('top')
                width = ign_box.getAttribute('width')
                height = ign_box.getAttribute('height')
                test_ign.extend([left,top,float(left)+float(width),float(top)+float(height)])
            # 获取所有的frame
            ...
            for frame in frames:
                ...
                for target in targets:
                    ...
                    seq_gt.append([frame_num, tar_id, x, y, float(width), float(height), type])   # (第几帧, 汽车id, 中心坐标, 宽, 高, 汽车类型)
        return seq_gt,test_ign
...
for seq in seqs:  # 每一个视频序列
    ...
    reader = XmlTester()
    gt, test_ign = reader.load(gt_xml)
    # 保存ignore_region
    test_ign.insert(0, seq)
    with open('/data0/yubo.xuan/DETRAC-dataset/test_ign.txt', 'a') as f:
        # 将每个列表转换为一个字符串，并写入文件中
        f.write(' '.join(str(x) for x in test_ign) + '\n')
    # 统计这个序列所有ID
    ids = []
    for line in gt:
    ...
    for fid, tid, x, y, w, h, label in final_gt:
        ...
        label_str = '{:d} {:.6f} {:.6f} {:.6f} {:.6f}\n'.format(label,
                                                                float(x) / seq_width, float(y) / seq_height,
                                                                float(w) / seq_width,
