...
def _process_transmission_process(transmission_queue, stream, i, j, device_ID, step_index, resolution, bitrate, width, height, episode):
    # 从这里开始计算cloud延迟
    timestamp = time.time()
    ...
    chunk = (chunk, timestamp, device_ID, i + 1, j + 1, height,
             width, step_index, episode)  # (frame, timestamp, deviceID, videoID, chunkID, height, width, step_index, episode)
...
def _sender(rtp_client_cloud, transmission_queue):
    ...
    while True:
        ...
        rtp_client_cloud.send_image_cloud(timestamp, payload, deviceID, videoID, chunkID,
                                                                        height, width, step_index,episode
                                                                        )
...
class EdgeClient:
    def __init__(self, args):
        ...
        if self.workload == 0:
            self.all_sleep_time = [1, 1, 1]
        elif self.workload == 1:
            self.all_sleep_time = [0.5, 0.5, 0.5]
        elif self.workload == 2:
            self.all_sleep_time = [1, 0.7, 0.5]
        elif self.workload == 3:  # 工作负载为从[1, 0.8, 0.6] 变到 [1, 1.1, 0.9] 变到 [1, 0.8, 0.6]
            self.all_sleep_time = [1, 0.75, 0.5]
        else:
            self.all_sleep_time = [1, 0.7, 0.5]

        self.sleep_time = self.all_sleep_time[0]
    ...
    def _process_inference_process(self, inference_queue, stream, i, j, device_ID, step_index, resolution, width,
        ...
        chunk = (frame, timestamp, device_ID, i + 1, j + 1, height,
                 width, step_index,
                 put_time, episode)  # (frame, timestamp, devic3ID, videoID, chunkID, height, width, step_index, 放入队列的时间, episode)
    ...
    def _load_camera_data(self):
        ...
        while True:
            ...
            print('all chunks=', str(self.num_chunks), '  cloud chunks= ', str(self.num_cloud_chunk), '  local chunks=',
                  str(self.num_local_chunk), '    videoID=', videoNum + 1, self.load_seq)
            j = 0
            while j < self.num_chunks:
                # 启动timer
                time_flag.value = 0
                self.timer.apply_async(func=timer,args=(self.sleep_time,))
                ...
                if self.load_seq[j] == 0:  # 表示处理边端数据
                    ...
                    self.encoder.apply_async(func=_process_transmission_process,
                                            ...
                                            self.resolution, self.bitrate, self.imgsz[1], self.imgsz[0], self.episode))
            ...
            if self.done_flag_double:  # 说明此时camera需要kill
                self.done_flag_double = False
    ...
    def _process(self):  # 处理listen得到的数据
        ...
        while True:
            ...
            self._edge_inference((timestamp, frame, self.edgedeviceid, videoID, chunkID, queue_wait, height, width, step_index, episode))
    ...
    def _control_client(self):
        ...
        def callback(ch, method, properties, body):
            ...
            if command['type'] == 'reset':
                ...
                # reset
                self.offload_rate = 0.4
                self.step_index = command['step']
                self.resolution = '640x360'
                ...
                if self.step_index == 0:
                    command_change = "ps -ef |grep ./change_bandwidth.sh |awk '{print $2}'|xargs kill -9"  # 替换为你想执行的Linux命令
                    process = subprocess.Popen(command_change, shell=True)
                    time.sleep(0.2)
                    command_change = "./change_bandwidth.sh &"  # 替换为你想执行的Linux命令
                    process = subprocess.Popen(command_change, shell=True)

                self.reset_thread = threading.Thread(target=self._load_camera_data)
                ...
                self.episode = command['episode']
                self.step_index = command['step']
                flag = command['flag']
                image = self.resolution.split('x')
                image = list(map(int, image))
                self.imgsz = (image[1], image[0])
                if self.step_index == 1301 and self.workload == 3:
                    self.all_sleep_time = [1, 0.9, 1.1]
                if self.step_index == 2601 and self.workload == 3:
                    self.all_sleep_time = [1, 0.75, 0.5]
                self.num_local_chunk = math.ceil(self.num_chunks * (1 - self.offload_rate))  # 在边缘端的块数
                self.num_cloud_chunk = int(self.num_chunks - self.num_local_chunk)
                self.load_seq = get_seq_random(self.num_local_chunk, self.num_cloud_chunk)

                self.sleep_time = self.all_sleep_time[((self.step_index-1) // 2) % len(self.all_sleep_time)]
...
if __name__ == '__main__':
    ...
    parser.add_argument("--workload", required=False, default=0, type=int,  # 0:正常负载1， 1:高负载2  2: 动态负载
                        help="the workload")
    # 解析命令行参数
    all_args = parser.parse_args()
    args = Config(all_args.workload)
    args.devicetype = 'NX'
