        ...
        while True:
            if not inference_queue.empty():
                ...
                for num in range(rounds):
                    ...
                    print("time====", latency)
                    # print(pred)
                    if len(frame_chunk[1]) == 30:
                        self.inference_latency.append(latency)
            else:
                print("avg time =====", np.mean(self.inference_latency))
                break
...
if __name__ == '__main__':
    ...
    parser.add_argument("--batchsize", required=False, default=30, type=int,
