...
class EdgeClient:
    ...
    #             # inference_process = stream.inference_process(data[2], resolution=self.resolution)  # 格式转化
    ...
    def _process_h264Tobgr24(self, inference_queue):  # 处理listen得到的数据
        ...
        while True:
            if self.flag:
                with open("dataset_acc_new.txt", "a") as file:
                    file.write(self.resolution + ', ' + self.bitrate + ', ' + self.modelimgsz + ', ' + str(np.mean(All_acc)) + '\n')
                break
            if inference_queue.qsize() > 0:
                data = inference_queue.get()  # (stream, videoNum, chunkNum, queue_timestamp)
                stream = data[0]
                queue_wait = time.time() - data[3]
                # print("======", queue_wait)
                (height, width) = self.imgsz
    ...
    def _edge_inference(self, frame_chunk):
        ...
        for num in range(rounds):
            ...
            start = time.time()
            pred = self.yolo.predict(inference_data, half=True)
            print(pred[0].boxes.data.cpu())
            ...
            for i in range(num * inference_batchsize, num * inference_batchsize + len(pred)):
                ...
                if len(labels_chunk[i]) == 0:  # label文件为空
                    ...
                    if len(pred[i - num * inference_batchsize]) == 0:
                        ...
                        # prediction = pred[0].boxes.data.cpu()
...
if __name__ == '__main__':
    ...
    parser.add_argument("--modelimgsz", required=False, default='960x960', type=str,
                        help='the encode bitrate')
