...
class EdgeClient:
    def __init__(self, args):
        ...
        modelsize = self.modelimgsz.split('x')
        modelsize = list(map(int, modelsize))
        self.modelsize = (modelsize[1], modelsize[0])
        ...
        self.yolo = YOLO(ROOT + model_dic[self.modelID], imgsz=self.modelsize)
        self.camera_dataset = Camera()
        self.load = threading.Thread(target=self._load_camera_data)
        self.load.start()

        self._process_h264Tobgr24(inference_queue)
    ...
    def _load_camera_data(self):
        ...
        # while True:
            ...
            j = 0
            while j < num_chunks:
                inference_queue.put((stream, videoNum, j, time.time()))
                j += 1
                time.sleep(1.5)
            i = i + 1
        time.sleep(1.5)
    ...
    def _process(self, inference_queue):  # 处理listen得到的数据
        ...
        while True:
            ...
            if inference_queue.qsize() > 0:
                ...
                # inference_process = stream.inference_process(data[2], resolution=self.resolution, bitrate=self.bitrate)  # 格式转化
                inference_process = stream.test1_process()  # 格式转化
    ...
    def _process_h264Tobgr24(self, inference_queue):  # 处理listen得到的数据
        print('start to process')
        while True:
            if self.flag:
                break
            if inference_queue.qsize() > 0:
                ...
                queue_wait = time.time() - data[3]
                (height, width) = self.imgsz
                transmission_process = stream.transmission_process(data[2], resolution=self.resolution, bitrate=self.bitrate)  # 格式转化
                edge_frame = transmission_process.stdout.read(width * height * 3 * 30)
                print(len(edge_frame))
                decode_process = (ffmpeg
                                  .input('pipe:', format='h264')
                                  .output('pipe:', format='rawvideo', pix_fmt='bgr24', loglevel='quiet')
                                  .run_async(pipe_stdin=True, pipe_stdout=True)
                                  )
                # decode_process.stdin.write(edge_frame)
                ...
                # print(frame.shape)
                self._cloud_inference((timestamp, frame, self.deviceID, data[1] + 1, data[2] + 1))
                inference_queue.task_done()
...
if __name__ == '__main__':
    ...
    parser.add_argument("--cloudport", required=False, default=1080, type=int,
                        help="the host port")
    parser.add_argument("--deviceid", required=False, default='00', type=str,
    ...
    parser.add_argument("--bitrate", required=False, default='5000000', type=str,
                        help='the encode bitrate')
    parser.add_argument("--modelimgsz", required=False, default='1280x720', type=str,
                        help='the encode bitrate')
