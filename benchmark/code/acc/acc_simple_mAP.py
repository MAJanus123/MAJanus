...
class EdgeClient:
    def __init__(self, args):
        ...
        self.video_acc = []
        model_dic = {'01': '/res/ultralytics/yolo/weights/yolov8n_trained_dynamic.engine',
                     '02': '/res/ultralytics/yolo/weights/yolov8s_trained.engine',
                     '03': '/res/ultralytics/yolo/weights/yolov8m_trained.engine',
                     '04': '/res/ultralytics/yolo/weights/yolov8l_trained.pt'}
    ...
    def _process_h264Tobgr24(self):  # 处理listen得到的数据
        ...
        time.sleep(1)
        # print(acc_dict)
        print("latency==", np.mean(latency[1:])*2)
        print("acc ==", np.mean(All_acc))
        # with open("acc_v8_mAP_modelL.txt", "a") as file:
        #     file.write(
        #         self.resolution + ', ' + self.bitrate + ', ' + self.modelimgsz + ', ' + str(np.mean(All_acc)) + ', ' + str(np.mean(latency[1:])*2) + ', ' + self.modelID + '\n')
    ...
    def _edge_inference(self, frame_chunk):
        ...
        for num in range(rounds):
            ...
            pred_bantch = self.yolo.predict(inference_data, half=True, imgsz=640)
        ...
        print("MQ_dic:  ", MQ_dic)
        # with open("upper_bound_r576x324P_b2500_cloud_modelL.txt", "a") as file:
...
if __name__ == '__main__':
    ...
    parser.add_argument("--modelID", required=False, default='01', type=str,
                        help='the model for inference')
    parser.add_argument("--resolution", required=False, default='960x540', type=str,
                        help='the encode resolution')
    parser.add_argument("--bitrate", required=False, default='3000000', type=str,
                        help='the encode bitrate')
    parser.add_argument("--modelimgsz", required=False, default='960x540', type=str,
                        help='the encode bitrate')
