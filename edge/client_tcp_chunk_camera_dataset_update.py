...
class EdgeClient:
    def __init__(self, args):
        ...
        self.sender = Pool(1)
        self.encoder = Pool(2)
        self.all_sleep_time = [[1, 0.8, 0.6], [1, 1.1, 0.95], [1, 0.8, 0.6]]
        self.sleep_time = 1
    ...
    def _process_inference_process(self, inference_queue, stream, i, j, device_ID, step_index, resolution, width,
        ...
        chunk = (frame, timestamp, device_ID, i + 1, j + 1, height,
                 width, step_index,
                 put_time, episode)  # (frame, timestamp, deviceID, videoID, chunkID, height, width, step_index, 放入队列的时间, episode)
    ...
    def _control_client(self):
        ...
        def callback(ch, method, properties, body):
            ...
            if command['type'] == 'reset':
                ...
                sleep_time_list = self.all_sleep_time[(self.step_index-1 // 910) % len(self.all_sleep_time)]
                self.sleep_time = sleep_time_list[(self.step_index-1 // 2) % len(sleep_time_list)]

                if self.step_index == 911:
                    command_change = "./change.sh &"  # 替换为你想执行的Linux命令
                    process = subprocess.Popen(command_change, shell=True)
                if self.step_index == 1821:
                    command_change = "./change_bandwidth.sh &"  # 替换为你想执行的Linux命令
                    process = subprocess.Popen(command_change, shell=True)
                if flag:
                    self.reset_thread = threading.Thread(target=self._load_camera_data)
